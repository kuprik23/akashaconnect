...
function dcl3Mint() {
    ...
    const mintingAc = useCallback(async (mintAmount,nTokenInpo) => {
      const   mintAmount2 = web3.utils.toWei(mintAmount + '');
        ...
        try {
            let nTokens;
            const res = await contract.methods.mintDCL3(nTokenInpo).send({
                from: account,
                value: mintAmount2
            })
            console.log('res of the dcl3Minting', res)
        } catch (error) {
         
            console.log('Error of the dcl3Minting', error)
            throw error;
        }
