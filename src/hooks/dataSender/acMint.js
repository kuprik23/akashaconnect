...
function acMint() {
    ...
    const mintingAc = useCallback(async (mintAmount,nTokenInpo) => {
      const   mintAmount2 = web3.utils.toWei(mintAmount + '');
        ...
        try {
            let nTokens;
            const res = await contract.methods.mintAC(nTokenInpo).send({
                from: account,
                value: mintAmount2
            })
            console.log('res of the acMinting', res)
        } catch (error) {
         
            console.log('Error of the acMinting', error)
            throw error;
        }
