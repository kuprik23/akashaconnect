...
function dcl4Mint() {
    ...
    const mintingAc = useCallback(async (mintAmount,nTokenInpo) => {
      const   mintAmount2 = web3.utils.toWei(mintAmount + '');
        ...
        try {
            let nTokens;
            const res = await contract.methods.mintDCL4(nTokenInpo).send({
                from: account,
                value: mintAmount2
            })
            console.log('res of the dcl4Minting', res)
        } catch (error) {
         
            console.log('Error of the dcl4Minting', error)
            throw error;
        }
