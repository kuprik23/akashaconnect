...
import { AkashaMintingContract } from '../../utils/contractHelpers'
 ...
 const CheckAcPrice = () => {
    ...
    const acPrice = useCallback(
        async (e) => {
            try {
                console.log(' acpreprice====contract===function')
                const res = await contract.methods.DCL4PrePrice().call();
                console.log(' acpreprice====contract===function====res',res)
    ...
    return { acPrice: acPrice };
};

export default CheckAcPrice;
