...
import { AkashaMintingContract } from '../../utils/contractHelpers'
 ...
 const GetCurrMintAmount = () => {
    const { account } = useWeb3React();
    const web3 = useWeb3();
    const tokenAddress = environment.mintContract;
    const contract = AkashaMintingContract(tokenAddress, web3);
    const curMintAmount = useCallback(
        async (e) => {
            try {
                console.log(' acpreprice====contract===function')
                const res = await contract.methods.getMintAmounts(account).call();
                console.log(' dcl4====contract===function====res',res)
    ...
    return { curMintAmount: curMintAmount };
};

export default GetCurrMintAmount;
